...
class Process:
    ...
    def getBurstTime(self):
        return self.burst_times[self.idx_burst]


class SJFScheduler:
    """
    SJP scheduler maintains a priority queue of processes to retrieve the shortest proces first.
    """
    ...
    def __init__(self, processes):
        ...
        for i in range(len(processes)):
            self.queue.put((processes[i].getBurstTime(), i))
        self.cpu = self.queue.get()[1]
    ...
    Puts the process into the priority queue.
    """

    def processReady(self, id):
        self.queue.put((self.processes[id].getBurstTime(), id))
    ...
    If the CPU is idle, it schedules the next shortest process in the priority queue.
